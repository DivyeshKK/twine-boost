...
const Index = () => {
  ...
  const features = [
    ...
    {
      icon: <Star className="text-twine-600" size={20} />,
      text: "Intelligently generate more positive customer reviews"
    },
    {
        ...
        <div className="step-indicator">
          {Array.from({ length: totalSteps }).map((_, idx) => (
            <div key={idx} className="flex items-center">
              <div 
                className={`step-dot ${currentStep > idx ? 'completed' : ''} ${currentStep === idx + 1 ? 'active' : ''}`} 
              />
              {idx < totalSteps - 1 && (
                <div 
                  className={`step-line ${currentStep > idx + 1 ? 'active' : ''}`} 
                />
              )}
            </div>
          ))}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updateFormData('business', 'size', e.target.value)}
                      >
                    ...
                    <div>
                      ...
                      <select
                        id="locations"
                        className="input-field"
                        value={formData.business.locations}
                        onChange={(e) => updateFormData('business', 'locations', e.target.value)}
                      >
                ...
                <motion.div variants={itemVariants} className="space-y-3">
                  <div className="grid grid-cols-1 gap-3">
                    <ChallengeOption
                      id="missedLeads"
                      icon={<Clock size={20} />}
                      label="Missing leads due to slow response times"
                      isSelected={formData.challenges.missedLeads}
                      onClick={() => handleChallengeToggle('missedLeads')}
                    />
                    
                    <ChallengeOption
                      id="inconsistentService"
                      icon={<Layers size={20} />}
                      label="Inconsistent customer service across locations"
                      isSelected={formData.challenges.inconsistentService}
                      onClick={() => handleChallengeToggle('inconsistentService')}
                    />
                    
                    <ChallengeOption
                      id="afterHoursSupport"
                      icon={<Clock size={20} />}
                      label="After-hours customer support challenges"
                      isSelected={formData.challenges.afterHoursSupport}
                      onClick={() => handleChallengeToggle('afterHoursSupport')}
                    />
                    
                    <ChallengeOption
                      id="channelManagement"
                      icon={<Globe size={20} />}
                      label="Complex lead management across multiple channels"
                      isSelected={formData.challenges.channelManagement}
                      onClick={() => handleChallengeToggle('channelManagement')}
                    />
                    
                    <ChallengeOption
                      id="scalingChallenges"
                      icon={<BarChart size={20} />}
                      label="Difficulty scaling customer service with growth"
                      isSelected={formData.challenges.scalingChallenges}
                      onClick={() => handleChallengeToggle('scalingChallenges')}
                    />
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <ChannelOption
                      ...
                      icon={<Phone size={20} />}
                      label="Phone Calls"
                      isSelected={formData.communication.channels.phone}
                    ...
                    <ChannelOption
                      ...
                      icon={<Mail size={20} />}
                      label="Email"
                      isSelected={formData.communication.channels.email}
                    ...
                    <ChannelOption
                      ...
                      isSelected={formData.communication.channels.sms}
                    ...
                    <ChannelOption
                      ...
                      icon={<Globe size={20} />}
                      label="Social Media"
                      isSelected={formData.communication.channels.socialMedia}
                      onClick={() => handleChannelToggle('socialMedia')}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                    <div>
                      ...
                      <select
                        id="leadVolume"
                        className="input-field"
                        value={formData.communication.leadVolume}
                        onChange={(e) => updateFormData('communication', 'leadVolume', e.target.value)}
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updateFormData('communication', 'responseTime', e.target.value)}
                      >
...
const ChallengeOption = ({ id, icon, label, isSelected, onClick }) => {
  return (
    <div
      onClick={onClick}
      className={`flex items-center p-4 rounded-lg border cursor-pointer transition-all duration-300 ${
        isSelected 
          ? 'bg-twine-50 border-twine-300 shadow-sm' 
...
const ChannelOption = ({ id, icon, label, isSelected, onClick }) => {
  return (
    <div
      onClick={onClick}
      className={`flex items-center p-4 rounded-lg border cursor-pointer transition-all duration-300 ${
